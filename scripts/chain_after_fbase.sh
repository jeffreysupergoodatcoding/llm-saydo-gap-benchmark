...
echo "[chain] Running Phase 10d D2-flat on the same core-1000 (full paired n for H7)..." | tee -a $LOG
PYTHONUNBUFFERED=1 PYTHONPATH=. uv run python scripts/phase10d_d2_on_core.py --per-bucket 200 --model gemini-2.5-flash 2>&1 | tee -a $LOG || true
