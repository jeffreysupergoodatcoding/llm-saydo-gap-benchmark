...
|---|---|---|---|---|---|---|
| **A. Gemini per-DP** (frontier LLM, per-decision API calls) | 1000 | 0.625 | 0.210 | **+0.421** | [+0.386, +0.456] | **+0.053** |
| **B. Claude meta-policy** (one-shot LLM-designed policy, deterministically applied) | 1000 | 0.354 | 0.210 | **+0.148** | [+0.112, +0.184] | **−0.052** |
| **C. Claude proper-DP v1** (per-customer per-DP LLM reasoning, deterministic sandbox) | 200 | 0.495 | 0.265 | **+0.227** | [+0.153, +0.305] | **+0.230** |
...
The within-bucket Spearman ρ column is the central finding. It varies by more than 0.28 across the four arms despite all four running the same M1 zero-shot protocol on overlapping customer samples.

**The cleanest test of the operationalization effect is the within-Claude contrast** (arms B vs C), which holds provider, prompt content, and dataset constant and varies only whether the LLM is invoked once-as-policy-designer or per-customer-per-decision-point. The wb-ρ swing is **−0.052 (meta-policy) → +0.230 (proper-DP)**, Δ = +0.282. With B = 1,000 paired bootstrap resamples within bucket on the n = 200 common customers, the 95% CI on this Δ is [+0.190, +0.385] (`results/phase43_cross_provider_analysis.json::paired_m1.claude_meta__vs__claude_per_dp::wb_rho_diff`), excluding zero by a wide margin. This is the operative claim of the paper: *operationalization-as-meta-policy vs operationalization-as-per-DP-reasoning produces a 0.28-point wb-ρ difference on the same Claude model running the same protocol, and the difference is statistically significant under paired stratified bootstrap*.

Three observations follow. **First, the within-Claude operationalization swing is on the same magnitude as the provider swing** (Gemini per-DP vs Claude per-DP v1 wb-ρ: +0.053 → +0.230 = 0.18 point); the available evidence does not allow us to rank them, but they are clearly on the same order. Both deserve attention from practitioners. **Second, environment dynamics matter modestly but do not collapse the operationalization effect.** Adding the real-world-dynamics sandbox (v2) drops wb-ρ from +0.230 to +0.202 — within sampling error of v1, well above the noise floor of meta-policy. The audit critique that the deterministic sandbox is "structured prompting" is partially defused by v2 showing the same qualitative finding holds under stochastic dynamics. **Third, the population-mean gap and within-bucket discrimination are separable.** Claude meta-policy has the smallest population-mean gap of any arm (+0.148) but the worst individual discrimination (wb-ρ = −0.052); Claude proper-DP v1 has a larger gap (+0.227) but the best individual discrimination (wb-ρ = +0.230). A practitioner picking between operationalizations is implicitly choosing which of these to optimize.

**Limitations of the four-arm framing.** The Gemini per-DP arm in this submission covers only M1, M2, M3 at n = 1,000 plus a partial M7 (n = 128); methods M8/M9/S1/S2/S3/S4 are not available on Gemini due to Google's 10K-request daily quota cap on both available API keys. The "four-arm" framing therefore describes the methodological design, not the empirical coverage; the operationalization claim above relies on the within-Claude contrast where coverage is matched.
